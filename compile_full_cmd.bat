@echo off
echo ========================================
echo Full Compilation (CMD + PowerShell)
echo ========================================
...
powershell -Command "Get-ChildItem -Path '%PROJECT_HOME%\src\main\java' -Recurse -Filter '*.java' | Select-Object -ExpandProperty FullName | ForEach-Object { '\"' + $_ + '\"' } | Set-Content -Path 'sources.txt' -Encoding Ascii"
